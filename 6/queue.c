 ...
 */
int queue_push(Queue* q, double x, double y) {
...
int queue_pop(Queue* q, Item* out) {
  ...
  out->x = q->items[q->head].x;
  out->y = q->items[q->head].y;
  q->head = (q->head + 1) % q->n;
...
void queue_test() {
  ...
  {
    queue_push(q, 1.0, 2.0);
    queue_push(q, 2.0, 3.0);
    queue_push(q, 3.0, 4.0);
    assert(!queue_push(q, 0.0, 0.0));

    queue_pop(q, &item);
    assert((item.x - 1.0 < e) && (item.y - 2.0 < e));
    queue_pop(q, &item);
    assert((item.x - 2.0 < e) && (item.y - 3.0 < e));
    queue_pop(q, &item);
    assert((item.x - 3.0 < e) && (item.y - 4.0 < e));
    assert(!queue_pop(q, &item));
